...
import { Mail, Github, Linkedin, Send, MessageSquare, CheckCircle, XCircle } from 'lucide-react';
...
const Contact = () => {
  ...
  const [formStatus, setFormStatus] = useState<{ status: 'idle' | 'sending' | 'success' | 'error'; message: string }>({ status: 'idle', message: '' });
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.current) {
      ...
      if (!serviceId || !templateId || !publicKey) {
        setFormStatus({ status: 'error', message: 'Email configuration is missing. Please check your .env file.' });
        return;
      }
      ...
      emailjs.sendForm(serviceId, templateId, form.current, publicKey)
        .then((result) => {
            ...
            setFormStatus({ status: 'error', message: `Something went wrong: ${error?.text || 'Check console for details'}` });
        });
    }
  };
                ...
                <form ref={form} onSubmit={handleSubmit} className="space-y-6 relative z-10">
                  ...
                  >
                    <span className="relative z-10 flex items-center justify-center gap-3 py-1">
                      ...
                      {formStatus.status !== 'sending' && <Send size={18} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />}
                    </span>
                  </motion.button>
